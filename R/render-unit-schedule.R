...
source(here::here("R", "format-homework-link.R"))
  ...
  titles <- schedule |>
    dplyr::filter(
      dplyr::when_any(
        ...
        unit == "tutorial" & type == "activity",
        unit %in% c("homework", "exam")
      )
    ) |>
    dplyr::mutate(
  ...
  unit_schedule <- units |>
    ...
    dplyr::mutate(
      slot = slot |>
        as.character() |>
        dplyr::replace_when(
          unit == "tutorial" & slot == "First" ~ "1^st^ slot",
          unit == "tutorial" & slot == "Second" ~ "2^nd^ slot"
        ),
      date = dplyr::if_else(
        unit == "tutorial",
        "",
