...
## Two views on Regular Expressions {.smaller}

| Meaning  | In Python/Pandas | Anchors |
|---|---|---|
| Full-string match | `re.fullmatch(r'(bb+|c[ac]*|b?)', s)`<br>`df['col'].str.fullmatch(r'(bb+|c[ac]*|b?)')` | Use `^` and `$`: `r'^(...)$'` |
| Substring search | `re.search(r'(bb+|c[ac]*|b?)', s)`<br>`df['col'].str.contains(r'(bb+|c[ac]*|b?)')` | none |
